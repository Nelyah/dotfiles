;;; tablericons.el --- AUCTeX style for `tablericons.sty' (v0.1.0)  -*- lexical-binding: t; -*-
...
;; This file adds support for `tablericons.sty' (v0.1.0) from
;; 2025-07-08.  `tablericons.sty' is part of TeXLive.
...
(defvar LaTeX-tablericons-filled-icons
  '("accessible"
    "ad-circle"
    "ad"
    "adjustments"
    ...
    "zoom-scan"
    "zoom")
  "List of filled icons form tablericons.sty.")

(defvar LaTeX-tablericons-outline-icons
  '("a-b-2"
    "a-b-off"
    "a-b"
    "abacus-off"
    ...
    "zzz-off"
    "zzz")
  "List of outline icons form tablericons.sty.")

(defun LaTeX-tablericons-filled-icon-p ()
  "Return non-nil if a filled tablericon is requested."
  (let ((p (point)))
    (save-excursion
      (and (re-search-backward "\\\\tablericon\\b"
                               (line-beginning-position -10) t)
           (string-match-p "filled\\(?:[ \t]*=[ \t]*true\\)?"
                           (buffer-substring-no-properties (point) p))
           (not (string-match-p "filled[ \t]*=[ \t]*false"
                                (buffer-substring-no-properties (point) p)))))))
...
(TeX-add-style-hook
 ...
 (lambda ()
   ...
   (TeX-add-symbols
    '("tablericon"
      [TeX-arg-key-val (("filled" ("true" "false"))
                        ("height" ("auto" "dauto"))
                        ("strut")
                        ("dstrut"))]
      [TeX-arg-key-val (LaTeX-graphicx-key-val-options)
                       "Graphics options" nil ?\s "<" ">"]
      (TeX-arg-completing-read (lambda ()
                                 (if (LaTeX-tablericons-filled-icon-p)
                                     LaTeX-tablericons-filled-icons
                                   LaTeX-tablericons-outline-icons))
                               "Icon name")))

   ;; Fontification
   (when (and (featurep 'font-latex)
              (eq TeX-install-font-lock 'font-latex-setup))
     (font-latex-add-keywords '(("tablericon" "[<{"))
                              'function)))
 TeX-dialect)

(defvar LaTeX-tablericons-package-options nil
  "Package options for the tablericons package.")
