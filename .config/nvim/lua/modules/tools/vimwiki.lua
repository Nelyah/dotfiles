...
local execute = function(cmd, raw)
...
function RecapDiaryWeek()
    ...
    local list_files =
    vim.fn.systemlist("ls " .. vim.g.vimwiki_soundhound.path .. '/diary/ -I diary.md -I "week-recap*"')
    ...
    -- Filter old files
    local list_index = 1
    local len_list_files = table_size(list_files)
    ...
    for _, diary_file in pairs(relevant_files) do
        ...
        diary_file_date = execute('date -d"' .. diary_file_date .. '" "+%A %b. %d %Y"')
